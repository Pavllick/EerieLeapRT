...
static std::vector<ext_unique_ptr<ISignal>> getSignals(const G_Network& gnet, const G_Message& m, Cache const& cache)
{
    std::vector<ext_unique_ptr<ISignal>> signals;
...
static std::vector<ext_unique_ptr<IMessage>> getMessages(const G_Network& gnet, Cache const& cache)
{
    ...
    for (const auto& m : gnet.messages)
    {
        ...
        {
            LOG_DBG("Message '%s' does have mux value but no mux signal!", msg->Name().c_str());
        }
