...
static eerie_leap::utilities::logging::Logger system_info_logger_("SystemInfo");
...
static void print_stack_info_callback(thread_analyzer_info* info) {
    ...
    system_info_logger_.Info("  %-14s: Unused %zu Usage %zu / %zu (%zu %%)",
        info->name,
...
void SystemInfo::print_stack_info(int cpu, const char *thread_name) {
    ...
    system_info_logger_.Info("Stack analyze for threads:");
    thread_analyzer_run((print_stack_info_callback), cpu);
#else
    system_info_logger_.Error("Thread analyzer is not supprted.");
#endif
}
...
static void print_cpu_info_callback(thread_analyzer_info *info) {
    ...
    system_info_logger_.Info("  %-14s: CPU Load: %u %%",
		info->name,
		info->utilization);
}
...
#ifdef CONFIG_THREAD_ANALYZER
    system_info_logger_.Info("CPU analyze for threads:");
...
static void print_heap_stats(sys_heap *heap) {
    ...
    system_info_logger_.Info("  %-14p: Unused %zu Usage %zu / %zu (%zu %%), Max Alloc %zu",
        heap,
        stats.free_bytes,
...
void SystemInfo::print_heap_info() {
    ...
    system_info_logger_.Info("Heap analyze, there are %zu heaps allocated at addrs:", heaps_count);
